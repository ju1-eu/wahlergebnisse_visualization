...
class ElectionVisualizer:
    ...
    def _draw_bars(self, scale: float = 1.0):
        ...
        for i, row in self.df.iterrows():
            if row["Harris"] > row["Trump"]:
                self.ax.plot(
                    [row["Trump"] * scale, row["Harris"] * scale],
                ...
                self.ax.plot(
                    [row["Harris"] * scale, row["Trump"] * scale],
                    [i + self.config.BAR_HEIGHT / 2] * 2,
                    color="darkgray",
                    linestyle=":",
                    alpha=0.7,
    ...
    def _add_value_label(
        self, value: int, percentage: float, scale: float, y_pos: float, color: str
    ):
        """
    ...
    def _configure_axes(self):
        ...
        # Titel und Labels
        self.ax.set_title(self.config.TITLE, fontsize=14, pad=20, fontweight="bold")
