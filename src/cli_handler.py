...
Beispiele:
  ...
  %(prog)s --no-cache --format png
        ...
        cache_group.add_argument(
            '--no-cache',
            action='store_true',
            help='Deaktiviert Daten-Caching'
        )
        cache_group.add_argument(
            '--clear-cache',
            action='store_true',
        ...
        dev_group.add_argument(
            '--profile',
            action='store_true',
            help='Aktiviert Performance-Profiling'
        )
    ...
    def _validate_args(self, args: Namespace):
        ...
        if not output_path.parent.exists():
            self.parser.error(f"Übergeordnetes Verzeichnis existiert nicht: {output_path.parent}")
        ...
        if args.no_cache and args.clear_cache:
            self.parser.error("--no-cache und --clear-cache können nicht zusammen verwendet werden")
