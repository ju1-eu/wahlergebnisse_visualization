...
class ElectionDataProcessor:
    ...
    def data(self) -> pd.DataFrame:
        ...
        if self._df is None:
            raise ValueError("Daten wurden noch nicht verarbeitet. Rufen Sie zuerst process_data() auf.")
        return self._df
    ...
    def _load_from_cache(self) -> Optional[pd.DataFrame]:
        ...
        if cache_path.exists():
            try:
                ...
                if (pd.Timestamp.now().timestamp() - modification_time) < self.cache_config.CACHE_EXPIRY:
